...
while IFS=$'\t' read -r date_and_session bag_file bag_files; do
    ...
    mkdir -p "/lab/tmpig23b/navisim/data/bag_dump/$date/$session/all_pcl/"
    rosrun pcl_ros bag_to_pcd "$bag_path" /velodyne_points "/lab/tmpig23b/navisim/data/bag_dump/$date/$session/all_pcl"

    sleep 10

    if [ -z "$(ls -A "/lab/tmpig23b/navisim/data/bag_dump/$date/$session/all_pcl/")" ]; then
